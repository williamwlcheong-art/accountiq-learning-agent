...
from main import _e2e_report_content, _render_report_sections_html, _validate_generated_report_content
...
from report_rendering import write_report_pdf
...
def test_market_content_adds_tables_and_chart_ready_series_to_both_reports():
    ...
    for report_type, section_key in (
        ...
        _validate_generated_report_content(enriched, report_type)
...
def test_report_html_renders_market_graphs_legends_sources_and_tables():
    ...
    rendered = _render_report_sections_html(
        content,
...
def test_report_pdf_renders_market_graphs_sources_and_sector_boundary(tmp_path):
    ...
    output = tmp_path / "market-intelligence.pdf"
    ...
    write_report_pdf(
        ...
        section_titles={"market_position": "Market Position"},
        report_id=27,
    ...
    assert "Annual CPI inflation and Official Cash Rate" in text
    assert "Retail Trade quarterly sales" in text
    assert "Stats NZ Business Financial Data" in text
    assert "Sector scale and boundary" in text
    assert "Four-quarter nominal sales proxy" in text
    assert "Market data sources" in text
