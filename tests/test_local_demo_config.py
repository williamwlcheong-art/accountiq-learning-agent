...
def test_env_example_keeps_anthropic_key_blank_and_documents_demo_mode():
    env_example = (ROOT / ".env.example").read_text(encoding="utf-8")

    assert "ANTHROPIC_API_KEY=sk-ant-YOUR_KEY_HERE" not in env_example
    assert "\nANTHROPIC_API_KEY=\n" in env_example
    ...
    assert "without a live Anthropic key" in env_example
