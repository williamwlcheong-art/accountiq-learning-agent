...
The `.planning/` docs are the current project source of truth. As of this context, `.planning/ROADMAP.md` and `.planning/PROJECT.md` say Phase 1 security/auth and Phase 2 multi-user isolation are complete. Older guide files may still describe those items as future work and should be reconciled before implementation planning.
...
7. **Frontend architecture decided:** decide whether the public site and app shell ship in Next.js before retiring the vanilla app, or whether a temporary split experience is acceptable.
...
Launch with **two or three products**, not all five possible report types.

Recommended initial product set:

1. **Business Valuation Report** — hero product and primary paid conversion.
2. **Forecasting Consultation / Forecast Report** — upsell from valuation, especially when valuation depends on future performance assumptions.
3. **Funding Readiness / Bank Credit Paper** — optional second report if fulfillment quality is ready; otherwise defer.
...
- Continue the Next.js frontend refactor for public site and app shell.
...
2. **Initial product count:** valuation only, valuation + forecasting, or valuation + forecasting + funding readiness?
...
6. **Infrastructure:** deploy Next.js + FastAPI on Railway with Postgres, or keep SQLite until commercial traffic forces migration?
...
10. **Guide reconciliation:** should stale `CLAUDE.md` / `AGENTS.md` project-guide state be updated now to prevent future planning confusion?

## Recommended Immediate Next Plans

Create these plans before implementation:

1. **Commercial MVP Architecture Plan**
   - Sequence Next.js, FastAPI, Stripe, dashboard, reviewer queue, and deployment.
