...
**Execution status:** Planning output completed. The Next.js foundation, runtime API proxy, and deterministic E2E mode are implemented on `main`; use `.planning/commercial/ARCHITECTURE-DECISIONS.md` for current decisions and do not re-run completed foundation steps.
...
**Tech Stack:** Next.js App Router, React, TypeScript, FastAPI, Python, Postgres, Railway, Stripe Checkout/Webhooks, PostHog, pytest, Playwright E2E.
...
- Next.js Route Handlers provide the runtime same-origin proxy used by the current app: `https://nextjs.org/docs/app/getting-started/route-handlers`
