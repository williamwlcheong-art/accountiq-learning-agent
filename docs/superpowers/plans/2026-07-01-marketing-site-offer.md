...
**Architecture:** Implement in the existing Next.js `web/` app. The marketing page is presentation-only except for privacy-safe analytics and a contact/consultation lead form; financial document upload remains behind authentication in FastAPI.

**Tech Stack:** Next.js App Router, React, TypeScript, the existing `web/app/globals.css` styling convention, PostHog with restricted explicit events, FastAPI for authenticated order/upload flow.
...
- The existing `web/` Next.js foundation and same-origin API route remain green in E2E.
...
Files to create or modify in the existing `web/` app:
...
```bash
pnpm test:e2e -- marketing.spec.ts
```
