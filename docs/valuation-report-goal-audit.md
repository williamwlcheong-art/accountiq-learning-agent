...
- A live provider-backed research/report smoke test has a harness, but has not been run in this environment because there is no real Anthropic API key configured.
...
2. Once a real Anthropic API key is available, run `python scripts/run_live_valuation_smoke.py` with `ACCOUNTIQ_DEMO_MODE=false` and audit the resulting JSON/HTML/PDF.
